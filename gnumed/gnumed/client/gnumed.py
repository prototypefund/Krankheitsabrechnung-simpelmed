...
def log_startup_info():
    ...
    for attr_name in dir(sys):
        ...
        if not attr_name.startswith('get'):
            ...
            continue
# getrefcount and getsizeof need an argument (inserted by kontakt@simpelmed.de)
##        if callable(attr):
##            try:
##                _log.info('%s: %s', attr_name.rjust(30), attr())
##            except Exception:
...
##            continue
    ...
    for attr_name in dir(platform):
        ...
        if callable(attr):
            if attr_name.startswith('_'):
                _log.info('%s: %s', attr_name.rjust(30), attr)
                continue
            # bug/error fixed; we do not need this information ...
            # (inserted by kontakt@simpelmed.de)
            if attr_name.rjust(30).endswith('system_alias'):
                _log.info('system_alias skipped')
                continue
            if attr_name.rjust(30).endswith('uname_result'):
                _log.info('uname_result skipped')
                continue
            # end bug/error fixed 
            try:
                _log.info('%s: %s', attr_name.rjust(30), attr())
    ...
    for n in os.sysconf_names:
        #### SC_EQUIV_CLASS_MAX not always available, we probably do not need this information here
        # (inserted by kontakt@simpelmed.de)
        if n.endswith('SC_EQUIV_CLASS_MAX'):
            _log.info('Skipped SC_EQUIV_CLASS_MAX')
            continue
        try:
