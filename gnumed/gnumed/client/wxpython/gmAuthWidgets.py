...
"""
SimpelMed Menu / Nutzerauthentifikation ...

Autoren
karsten.hilbert@gmx.net, H.Herb, H.Berger, R.Terry

Modifikationen des Originals von
...
#================================================================
__license__ = "AGPLv3 or later (details at http://www.gnu.org)"
...
## DEBUG: Aktuellen Import-Pfad loggen
#_log.debug('gmAuthWidget.py Zeile 43; syspath: %s', sys.path)
##
...
## wird zum Start von SimpelMed aufgerufen von gmGuiMain wie folgt:
...
##        )
    ...
    #input("gmauth unmitelbar vor Init des Anmeldefensters dlg 127")

    attempt = 0

    dlg = cLoginDialog(None, -1, client_version = client_version)
    dlg.Centre(wx.BOTH)
    #input("gmauth unmitelbar nach Init des Anmeldefensters dlg 133")
    while attempt < max_attempts:
        _log.debug('login attempt %s of %s', (attempt+1), max_attempts)
        connected = False
        # dlg.ShowModal() zeigt das »Anmeldefenster«
        # »ShowModal« -> blockt jedwede andere Aktion
        dlg.ShowModal()
        #input("gmauth nach Beendigung des Anmeldefensters 140; hole die Anmeldedaten")
        login = dlg.panel.GetLoginInfo()
        #print("LOGIN.user:                      ", login.user)
        #print("LOGIN.password:                  ", login.password)
        # login.host ist leer '', falls Socket genutzt wird
        #print("LOGIN.host (leer, falls Socket): ", login.host)
        #print("LOGIN.database:                  ", login.database)
...
##            port = profile.port
        ...
        # Die Klasse cPGCredentials() initialisiert alle ihre fünf Attribute mit None:
        # database, host, port, user, password
        # Übernahme zB aus dem Anmeldefenster
...
class cLoginDialog(wx.Dialog):
    ...
    def __init__(self, parent, id, title = "Willkommen zu SimpelMed! ", client_version = '*** unknown ***'):
...
class cLoginPanel(wx.Panel):
    ...
    def __init__(self, parent, id,
        ...
        paramsbox_caption = 'Arbeitsplatz "%s" (version %s)' % (gmPraxis.gmCurrentPraxisBranch().active_workplace, client_version)
        ...
        #!#
        # --slave checkbox / Experimental: Foto des Tages anzeigen
        # Dieser Block kann nach DEBUG gelöscht werden (alles zwischen den #!#) ... 
        #label = wx.StaticText(self, -1, '', wx.DefaultPosition, wx.DefaultSize, 0)
        #label.SetForegroundColour(wx.Colour(35, 35, 142))
        #self.pboxgrid.Add(label, 0, wx.ALIGN_CENTER_VERTICAL|wx.ALL, 5)
        #self._CHBOX_slave = wx.CheckBox(self, -1, 'Weltraum!')
        #self._CHBOX_slave.SetToolTip('Zeige ein spaciges Foto an!.')
        #self.pboxgrid.Add(self._CHBOX_slave, 0, wx.GROW | wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5)
        #!#
